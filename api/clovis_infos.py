...
clovis_infos (voir migration clovis_infos_singleton_et_type_clovis).
"""
...
def obtenir(utilisateur=Depends(utilisateur_courant)):
    res = supabase.table("clovis_infos").select("etoiles_count").eq("id", ID_CLOVIS).maybe_single().execute()
