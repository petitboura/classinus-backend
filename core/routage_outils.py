...
def _router_outils(message_utilisateur, outils_disponibles, historique=None):
    ...
    prompt_routeur = (
        ...
        "lui-même (pas juste son contenu/résumé/explication), distingue "
        "deux cas. (1) Fichier dans SA bibliothèque personnelle (monde 1), "
        "dans le catalogue public (monde 3), OU déjà GÉNÉRÉ PAR TOI plus "
        "tôt dans cette même conversation (image, document, audio, "
        "vidéo...) -- un fichier que tu génères est automatiquement "
        "enregistré dans la bibliothèque personnelle de l'utilisateur, "
        "donc retrouvable ensuite : suggère TOUJOURS gerer_document_bibliotheque. "
        "(2) Fichier ENVOYÉ (uploadé) par l'utilisateur PLUS TÔT DANS "
        "CETTE CONVERSATION (14/09/2026, correction : ce cas n'est PAS "
        "gerer_document_bibliotheque, qui exclut volontairement ces "
        ...
        "une information. Exemples qui DOIVENT suggérer "
        "gerer_document_bibliotheque : \"donne-moi le PDF sur les "
        "intégrales\", \"envoie-moi ce document de la bibliothèque "
        "publique\", \"redonne-moi l'image que tu as générée tout à "
        "l'heure\", \"renvoie-moi le document que tu m'as fait plus tôt\". "
        "Exemples qui DOIVENT suggérer gerer_fichier_conversation : "
        "\"renvoie-moi le fichier que je viens de t'envoyer\", "
        "\"redonne-moi la photo que je t'ai envoyée tout à l'heure\". "
